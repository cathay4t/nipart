...
pub enum InterfaceState {
    ...
    Unknown,
    /// Interface/profile only exists in saved config and is not activated.
    /// Query only state generated by the daemon running state query; it is
    /// not accepted in desired state.
    /// Deserialize and serialize from/to 'saved'.
    Saved,
}
