...
use super::{
    ...
    vlan::apply_vlan_conf, vxlan::apply_vxlan_conf, wireguard::apply_wg_conf,
};
use crate::{
...
pub(crate) fn apply_iface_link_changes(
    ...
    let apply_iface = merged_iface.for_apply.as_ref().ok_or_else(|| {
        NipartError::new(
            ErrorKind::InvalidArgument,
            format!(
                "BUG: Missing for_apply interface for {}/{}",
                merged_iface.merged.name(),
                merged_iface.merged.iface_type()
            ),
        )
    })?;
    if should_skip_link_change(
        apply_iface,
    ...
    } else if let Interface::Vlan(i) = apply_iface {
        apply_vlan_conf(np_iface, i)
    } else if let Interface::Vxlan(apply_iface) = apply_iface {
        apply_vxlan_conf(np_iface, apply_iface)
    } else if let Interface::Bond(apply_iface) = apply_iface {
        apply_bond_conf(np_iface, apply_iface, merged_iface, merged_ifaces)
    } else if let Interface::LinuxBridge(apply_iface) = &apply_iface {
        apply_bridge_conf(
            np_iface,
            apply_iface,
            if let Some(Interface::LinuxBridge(c)) =
                merged_iface.current.as_ref()
            {
                Some(c)
            } else {
    ...
    } else if let Interface::Vrf(vrf_iface) = apply_iface {
        if let Some(vrf_conf) = vrf_iface.vrf.as_ref() {
            let mut np_vrf_conf = nispor::VrfConf::default();
            np_vrf_conf.table_id = vrf_conf.table_id;
            np_iface.vrf = Some(np_vrf_conf);
        }
        Ok(vec![np_iface])
    } else {
        Ok(vec![np_iface])
