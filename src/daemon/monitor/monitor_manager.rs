...
impl NipartMonitorManager {
    ...
    pub(crate) async fn setup_monitor(
            ...
            .filter_map(|m| m.for_apply.as_ref())
        {
            ...
            } else if let Some(InterfaceTrigger::Carrier(_)) =
                iface.base_iface().up_trigger.as_ref()
            {
                self.add_iface_to_monitor(iface.name()).await?;
            } else if let Some(InterfaceTrigger::Carrier(_)) =
                iface.base_iface().down_trigger.as_ref()
            {
                self.add_iface_to_monitor(iface.name()).await?;
            }
        }
...
fn wifi_monitor_is_needed(full_saved_state: &NetworkState) -> bool {
    for iface in full_saved_state.ifaces.iter().filter(|i| !i.is_absent()) {
        ...
        {
            return true;
        } else if let Some(up_trigger) = iface.base_iface().up_trigger.as_ref()
            && up_trigger.is_wifi()
        {
            return true;
        } else if let Some(down_trigger) =
            iface.base_iface().down_trigger.as_ref()
            && down_trigger.is_wifi()
        {
            return true;
        }
    }
