...
use super::super::{
    daemon::NipartManagerCmd, event::NipartLinkEvent, task::TaskWorker,
};
...
impl NipartMonitorWorker {
    ...
    async fn resume(&mut self) -> Result<(), NipartError> {
        ...
        while let Some(Ok(link_msg)) = link_handle.next().await {
            if let Some(event) =
                parse_link_msg(&link_msg, self.wifi_monitor_enabled)
        ...
        })
    {
        ...
        log::trace!(
            "parse_link_msg() ignoring netlink message due to lack of \
             LinkAttribute::OperState or LinkAttribute::Wireless in \
             {link_msg:?}",
        );
        None
    } else {
...
fn parse_route_netlink_msg(
    ...
    {
        parse_link_msg(&link_msg, wifi_monitor_enabled)
    } else {
        None
    }
}
