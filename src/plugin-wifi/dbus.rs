...
impl NipartWpaSupDbus<'_> {
    pub(crate) async fn new() -> Result<Self, NipartError> {
        let builder = zbus::connection::Builder::system().map_err(|e| {
            NipartError::new(
                ErrorKind::Bug,
                format!("Failed to create system DBUS connection builder: {e}"),
            )
        })?;
        let connection =
            timeout(DBUS_TIMEOUT, builder.method_timeout(DBUS_TIMEOUT).build())
                .await
                .map_err(|_| {
                    NipartError::new(
                        ErrorKind::Bug,
                        "Timed out connecting to system DBus".to_string(),
                    )
                })?
                .map_err(|e| {
                    NipartError::new(
    ...
    pub(crate) async fn wait_scan(
        ...
        let mut stream = proxy
            ...
            .await
            .map_err(map_zbus_err)?;
        timeout(DBUS_TIMEOUT, stream.next()).await.map_err(|_| {
            NipartError::new(
                ErrorKind::Bug,
