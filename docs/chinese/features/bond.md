...
注意：大多数 Bond 选项仅在特定的绑定模式下有效（请参阅下面的每个选项），
上面的示例仅包含 `802.3ad` 模式下有效的选项。
...
802.3ad 用户定义的端口密钥。有效范围为 0 到 1023，默认为 0。
仅在 `802.3ad` 模式下有效。
...
之前在一个端口上传输的数据包数。仅在 `balance-rr` 模式下有效；最大值为
65535，默认为 1。
...
故障切换后发送的免费 ARP 数据包数。有效范围为 0 到 255，默认为 1。
如果同时定义了 `num_unsol_na`，两者必须相等。

### `num_unsol_na`：非请求 NA 数量

故障切换后发送的非请求 IPv6 邻居通告数。有效范围为 0 到 255，默认为 1。
内核中与 `num_grat_arp` 含义相同。
