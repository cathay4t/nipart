...
查询器发送的通用组播查询之间的间隔（毫秒）。内核会将值限制在 1000 到
86400000（24 小时）范围内；默认为 12500。
...
启动阶段发送的查询之间的间隔（毫秒）。内核会将值限制在 1000 到
86400000（24 小时）范围内；默认为 3125。
...
STP 网桥优先级。有效范围为 0 到 65535。较低的优先级会增加成为根网桥的机会。
...
STP 端口优先级。有效范围为 0 到 63（内核 `BR_MAX_PORT_PRIORITY`，优先级
占用 16 位端口 ID 的高 6 位）。较低的优先级会增加成为指定端口的机会。
